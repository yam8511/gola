...
func (遊戲 *Game) 天亮請睜眼() {
	...
	if len(遊戲.淘汰者) > 0 {
		...
		遊戲.旁白(傳輸資料{
			Sound:  "昨晚 " + strings.Join(死者名單號碼, ",") + " 玩家淘汰!",
			Action: 等待回應,
		}, 100)
...
func (遊戲 *Game) 大家開始發言(玩家們 []Player) {
	if len(玩家們) == 0 {
		玩家們 = 遊戲.存活玩家們()
	}
...
func (遊戲 *Game) 全員請投票() {
	...
	for 玩家號碼, 票數 := range 投票統計結果 {
		if 票數 == 最高票數 {
			...
			if 存在 {
				遊戲.殺玩家(票殺, 玩家)
				遊戲.旁白(傳輸資料{Sound: strconv.Itoa(玩家號碼) + " 淘汰! 請發表遺言"}, 0)
				玩家.發表遺言()
			}
			break
		}

...
func (遊戲 *Game) 天黑請閉眼() {
	...
	// 狼美人請睜眼
	if len(有狼美人) > 0 {
		遊戲.旁白(傳輸資料{Sound: "狼美人請睜眼"}, 3000)
		遊戲.旁白(傳輸資料{Sound: "狼美人請指定要睡的對象"}, 1000)
		wg := sync.WaitGroup{}
		for i := range 有狼美人 {
			if !有狼美人[i].出局了() {
				wg.Add(1)
...
func (遊戲 *Game) 天亮請睜眼() {
	...
	if 遊戲.有熊叫 {
		遊戲.旁白(傳輸資料{
			Sound:   "天亮請睜眼。遠方來了一陣熊叫聲！吼吼吼",
			Display: "天亮請睜眼。遠方來了一陣熊叫聲！吼~吼~吼 🐻",
			Action:  天亮請睜眼}, 6000)
	} else {
...
func (遊戲 *Game) 提示發言() string {
	if 遊戲.可自爆 {
		return "(狼人發動可自爆(狼美人除外)，騎士發動可查驗，其他則直接結束發言)"
	}
